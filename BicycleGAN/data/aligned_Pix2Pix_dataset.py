...
class AlignedPix2PixDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        season              = opt.sen12mscr_season
        s1_rescale_method   = opt.s1_rescale_method
        s2_rescale_method   = opt.s2_rescale_method
    ...
    def __getitem__(self, index):
        ...
        B_transform = get_transform(self.opt, transform_params, grayscale=(self.output_nc == 1), use_hsv_aug=self.opt.use_hsv_aug, use_gray_aug=self.opt.use_gray_aug, use_gaussian_blur=self.opt.use_gaussian_blur, kernel_size=self.opt.kernel_size, rescale_method=self.opt.s2_rescale_method)
