...
class SEN12MSCR(Dataset, ABC):
    def __init__(self, root, split="all", region='all', season='all', rescale_method='default'):
        ...
        assert split in ['all', 'train', 'val', 'test', 'test,val'], "Input dataset must be either assigned as all, train, test, val or test,val!"
    ...
    def __getitem__(self, pdx):  # get the triplet of patch with ID pdx
        ...
        coord           = list(s2_tif.bounds)
        s1              = process_SAR(read_img(s1_tif), self.method)
        s1              = np.vstack((s1[[0]], s1[[1]], (s1[[0]]+s1[[1]])/2.0)) # Add dummy third channel
    ...
    def get_SAR(self, pdx):  # get the triplet of patch with ID pdx
        s1_tif          = read_tif(os.path.join(self.root_dir, self.paths[pdx]['S1']))
        # s2_cloudy_tif   = read_tif(os.path.join(self.root_dir, self.paths[pdx]['S2_cloudy']))
    ...
    def get_EO(self, pdx):  # get the triplet of patch with ID pdx
        s2_tif          = read_tif(os.path.join(self.root_dir, self.paths[pdx]['S2']))
        coord           = list(s2_tif.bounds)
        s2              = read_img(s2_tif)[[3,2,1],:,:]# note: pre-processing happens after cloud detection / B4, B3, B2
        # s2_cloudy       = read_img(s2_cloudy_tif)    # note: pre-processing happens after cloud detection
